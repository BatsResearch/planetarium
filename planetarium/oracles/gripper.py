...
class GripperOracle(oracle.Oracle):
    ...
    def _gripper_underspecified_blocks(
        ...
        typed = GripperOracle._gripper_get_typed_objects(init)
    ...
    def fully_specify(
        ...
        init: ReducedSceneGraph = GripperOracle.reduce(inflated_init)
        goal: ReducedSceneGraph = GripperOracle.reduce(inflated_goal)

        scene = copy.deepcopy(goal)

        # bring "typing" predicates from init to goal
        typed_objects = GripperOracle._gripper_get_typed_objects(init)
        for typing, objects in typed_objects.items():
            for obj in objects:
                edge = graph.PlanGraphEdge(
                    predicate=typing.value, scene=graph.Scene.GOAL
                )
                edge_ = graph.PlanGraphEdge(predicate=typing.value)
                if obj in scene.nodes and not (
                    scene.has_edge(typing, obj, edge)
                    or scene.has_edge(typing, obj, edge_)
                ):
                    scene.add_edge(typing, obj, edge)

        underspecified_balls, underspecified_grippers, _ = (
            GripperOracle._gripper_underspecified_blocks(
                init,
                goal,
            )
        )
        ...
        if return_reduced:
            return ReducedProblemGraph.join(init, scene)
        else:
            return graph.ProblemGraph.join(inflated_init, GripperOracle.inflate(scene))
    ...
    def plan(problem: graph.ProblemGraph) -> list[Action]:
        ...
        problem = GripperOracle.fully_specify(problem, return_reduced=True)
        ...
        for gripper in grippers:
            if not init.has_edge(ReducedNode.FREE, gripper):
                ...
                ball = [
                    b
                    for b in init.predecessors(gripper)
                    if b in typed[ReducedNode.BALLS]
                ]
                if ball:
                    actions.append(
        ...
        for room in rooms:
            for obj in init.predecessors(room):
                if obj in typed[ReducedNode.BALLS]:
                    ...
                    actions.append(
                        Action("pick", [obj.name, room.name, grippers[0].name])
                    )
        ...
        for room in rooms:
            for obj in goal.predecessors(room):
                if obj in typed[ReducedNode.BALLS]:
                    ...
                    actions.append(Action("move", [rooms[0].name, room.name]))
                    actions.append(
                        Action("drop", [obj.name, room.name, grippers[0].name])
                    )
                    actions.append(Action("move", [room.name, rooms[0].name]))
        ...
        for gripper in grippers:
            for ball in typed[ReducedNode.BALLS]:
                if goal.has_edge(ball, gripper):
                    actions.append(
                        Action("pick", [ball.name, rooms[0].name, gripper.name])
                    )
