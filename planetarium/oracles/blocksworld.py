...
class BlocksworldOracle(oracle.Oracle):
    ...
    def fully_specify(
        ...
        scene = BlocksworldOracle.reduce(inflated_goal)
        top_blocks, bottom_blocks, arm_empty = (
            BlocksworldOracle._blocksworld_underspecified_blocks(scene)
        )
        top_blocks_, bottom_blocks_ = BlocksworldOracle._detached_blocks(
            top_blocks,
            bottom_blocks,
            scene,
        )
        ...
        if return_reduced:
            return ReducedProblemGraph.join(
                BlocksworldOracle.reduce(inflated_init), scene
            )
        else:
            return graph.ProblemGraph.join(
                inflated_init, BlocksworldOracle.inflate(scene)
            )

    def plan(problem: graph.ProblemGraph) -> list[Action]:
        problem = BlocksworldOracle.fully_specify(problem, return_reduced=True)
        ...
        for idx in reversed(rx.topological_sort(goal.graph)):
            ...
            if isinstance(node.node, ReducedNode):
                ...
                actions.append(
                    Action("stack", [node.name, goal.successors(node)[0].name])
                )
